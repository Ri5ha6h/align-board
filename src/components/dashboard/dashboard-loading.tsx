...
export function DashboardSkeleton({ className }: { className?: string }) {
	return (
		<span
			aria-hidden="true"
			className={cn("dashboard-shimmer block rounded-[2px]", className)}
		/>
	);
}
	...
	return (
		<div aria-label="Loading dashboard data" aria-live="polite" className="mt-6 space-y-px">
			{Array.from({ length: 6 }, (_, index) => (
				<div
					className="grid min-h-14 grid-cols-[1.1fr_0.8fr_2fr_1.4fr] items-center gap-5 border border-[#444449] px-4"
					key={index}
				>
